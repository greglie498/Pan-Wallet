...
class TransactionService {
  ...
  private async buildQuote(
    ...
    return {
      ...
      senderCurrency,
      recipientProvider,
      recipientNumber, // Fixed empty string issue
      amount,
      convertedAmount,
      exchangeRate: rate,
      fee,
  ...
  async initiateTransfer(
    ...
    if (senderWallet.balance.lessThan(requiredAmount)) {
      throw new BadRequestError(
        `Insufficient balance. Available: ${senderWallet.currency} ${senderWallet.balance.toFixed(
          2
        )}. Required: ${senderWallet.currency} ${requiredAmount.toFixed(2)}.`
      );
    }
    ...
    const { transaction } = await prisma.$transaction(async (tx) => {
      ...
      if (!debited) {
        throw new BadRequestError(
          "Insufficient balance for this transfer and its fee."
        );
      }
    ...
    try {
      ...
      if (env.NODE_ENV === "development") {
        logger.warn(
          "Provider call failed in sandbox — auto-completing transaction.",
          {
      ...
      // Production — mark as failed and refund
  ...
  async handleCallback(result: CallbackResult): Promise<void> {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      // Safe Prisma Decimal instantiation
      const refund = new Prisma.Decimal(transaction.amount.toString()).plus(
        new Prisma.Decimal(transaction.fee.toString())
      );
